...
const Reports = () => {
  ...
  const filteredRequests = requests.filter(request => {
    ...
    return employee?.service_id === parseInt(selectedService);
  });

  // Calculer les statistiques
  const stats = {
    totalRequests: filteredRequests.length,
    totalHours: filteredRequests.reduce((sum, req) => sum + (req.hours || 0), 0),
    approvedRequests: filteredRequests.filter(req => req.status === 'Approved').length,
    approvedHours: filteredRequests.filter(req => req.status === 'Approved').reduce((sum, req) => sum + (req.hours || 0), 0),
  ...
  const serviceStats = services.map(service => {
    ...
    return {
      ...
      totalHours: serviceRequests.reduce((sum, req) => sum + (req.hours || 0), 0),
      approvedHours: serviceRequests.filter(req => req.status === 'Approved').reduce((sum, req) => sum + (req.hours || 0), 0),
    };
  });
  ...
  const employeeStats = employees.map(employee => {
    ...
    return {
      ...
      totalHours: employeeRequests.reduce((sum, req) => sum + (req.hours || 0), 0),
      approvedHours: employeeRequests.filter(req => req.status === 'Approved').reduce((sum, req) => sum + (req.hours || 0), 0),
    };
